...
void CivilVerification_3_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void CivilVerification_3_run(uint ctx_index,Circom_CalcWit* ctx);
Circom_TemplateFunction _functionTable[4] = { 
Num2Bits_0_run,
LessThan_1_run,
GreaterThan_2_run,
CivilVerification_3_run };
...
uint get_total_signal_no() {return 791;}
...
ctx->componentMemory[coffset].templateName = "CivilVerification";
ctx->componentMemory[coffset].signalStart = soffset;
...
int cmp_index_ref_load = -1;
{
std::string new_cmp_name = "minAgeCheck";
GreaterThan_2_create(mySignalStart+270,3+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[0] = 3+ctx_index+1;
}
{
std::string new_cmp_name = "maxAgeCheck";
GreaterThan_2_create(mySignalStart+10,0+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[1] = 0+ctx_index+1;
}
{
std::string new_cmp_name = "monthlyIncomeCheck";
GreaterThan_2_create(mySignalStart+530,6+ctx_index+1,ctx,new_cmp_name,myId);
...
cmp_index_ref_load = 1;
Fr_mul(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[0]].signalStart + 0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[1]].signalStart + 0]); // line circom 31
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_sub(&expaux[2],&circuitConstants[2],&signalValues[mySignalStart + 8]); // line circom 40
Fr_mul(&expaux[1],&expaux[2],&signalValues[mySignalStart + 6]); // line circom 40
Fr_sub(&expaux[2],&circuitConstants[2],&signalValues[mySignalStart + 6]); // line circom 40
Fr_add(&expaux[0],&expaux[1],&expaux[2]); // line circom 40
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
for (uint i = 0; i < 3; i++){
...
CivilVerification_3_create(1,0,ctx,"main",0);
CivilVerification_3_run(0,ctx);
}
